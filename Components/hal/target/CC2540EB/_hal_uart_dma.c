...
//DMA_PM  开启电源管理此项为1
  if (DMA_PM)
  {
    ...
    PxSEL &= ~( HAL_UART_Px_CTS && HAL_UART_Px_RTS ); //P0.4、P0.5设置成GPIO
    PxDIR &= ~DMA_RDYIn_BIT;  //P0.4设置输入
    P0_4 = 1;
    PxIEN |= DMA_RDYIn_BIT;   //开启P0.4中断
    PICTL |= PICTL_BIT;       //设置中断方式为下降沿
    PxDIR |= DMA_RDYOut_BIT;  //P0.5设置成输出
...
static void HalUARTOpenDMA(halUARTCfg_t *config)
{
  ...
  HAL_ASSERT(     (config->baudRate == HAL_UART_BR_4800)  ||
...
#endif
      {
        //HAL_UART_DMA_SET_RDY_OUT();//P0.5=0
      }
      dmaRdyIsr = 0;
